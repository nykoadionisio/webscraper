...
def site_information(web: Website, keywords: list[str]) -> WebInfo:
    ...
    for i in range(0, len(hrefs)):
        ...
        webinfo = WebInfo(source, keyword_frequency, total_words)
        return webinfo


def get_webinfo_list(websites: list[Website], keywords: list[str]) -> list[WebInfo]:
    """ Converts a list of Website types to a list of WebInfo types"""
    new_sites = []
    for website in websites:
        converted_info = site_information(website, keywords)
        new_sites.append(converted_info)
    return new_sites
...
def main():
    ...
    national = Website('https://nationalpost.com', 'a', 'article-card__link', 'section',
                       'article-content__content-group')
    ...
    webinfo = get_webinfo_list(websites, keywords)
    trends = trends_data(keywords)
    find_related(webinfo, keywords)
